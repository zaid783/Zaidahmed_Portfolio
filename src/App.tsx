...
function App() {
    ...
    useEffect(() => {
        const trackVisitor = async () => {
            ...
            let visitorData: any = {
                Device: `${navigator.platform} (${window.screen.width}x${window.screen.height})`,
                System_Info: navigator.userAgent,
                Time: new Date().toLocaleString(),
            };

            try {
                // 1. Get Location Info (Using ipwho.is - more CORS friendly)
                console.log('Fetching location data from ipwho.is...');
                const locResponse = await fetch('https://ipwho.is/');
                const locData = await locResponse.json();

                if (locData.success) {
                    console.log('Location data received:', locData.city, locData.country);
                    visitorData = {
                        ...visitorData,
                        _subject: `🚀 Portfolio Visit: ${locData.city}, ${locData.country}`,
                        Location: `${locData.city}, ${locData.region}, ${locData.country}`,
                        IP: locData.ip,
                        Organization: locData.connection?.org || 'Unknown'
                    };
                } else {
                    console.warn('Geolocation lookup failed, sending basic info.');
                    visitorData._subject = '🚀 Portfolio Visit: Unknown Location';
                }
            } catch (error) {
                console.error('Geolocation fetch error:', error);
                visitorData._subject = '🚀 Portfolio Visit: Tracking error (Location Blocked)';
            }

            try {
                // 2. Send to FormSubmit
                console.log('Sending data to FormSubmit...', visitorData);
                const response = await fetch('https://formsubmit.co/ajax/zaidlaiq99@gmail.com', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                    body: JSON.stringify({
                        ...visitorData,
                        Message: 'A visitor has landed on your portfolio.'
                    })
                });
                ...
                console.error('FormSubmit send failed:', formError);
            }
        };
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="glass-effect p-8 rounded-2xl border border-cyan-500/20 hover:border-cyan-500/40 transition-all duration-500 transform hover:scale-[1.02] hover:shadow-[0_0_40px_rgba(34,211,238,0.3)]">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="text-slate-400 text-sm bg-slate-800/50 px-4 py-2 rounded-full">Dec 2024 - March 2025</span>
                                </div>
                                <ul className="space-y-3 text-slate-300">
